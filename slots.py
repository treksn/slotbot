...
def get_slots(date_time: str) -> str:
    ...
    if len(date_time) > 2:
        ...
        if slot.available_slots == 0:
            message += '\n' + '-> Next free slots are:'
            slot_previous = get_next_available_slot(selected_slot_idx, slots, True)
        ...
        if len(slots) == 0:
            message += print('No slots available for', select_time.strftime('%d.%m.')) + '\n'
...
def parse_weekday(weekday: str) -> datetime:
    ...
    desired_weekday = weekdays.index(weekday.lower())
...
def print_slot(slot: Slot) -> str:
    slot_expression = "slot" if slot.available_slots == 1 else "slots"
    return f"{local_time(slot.datetime)} - {slot.available_slots} {slot_expression}."
